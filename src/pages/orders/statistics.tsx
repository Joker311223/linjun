import React, { useState, useEffect } from 'react';
import { Card, Row, Col, Statistic, DatePicker, Select, Divider, Spin, Radio } from 'antd';
import { ArrowUpOutlined, ArrowDownOutlined, DollarOutlined, ShoppingCartOutlined, UserOutlined, LineChartOutlined } from '@ant-design/icons';
import axios from 'axios';
import dayjs from 'dayjs';
import ReactECharts from 'echarts-for-react';

const { RangePicker } = DatePicker;
const { Option } = Select;

interface StatisticsData {
  totalAmount: number;
  totalOrders: number;
  totalUsers: number;
  averageAmount: number;
  comparedToLastPeriod: {
    amount: number;
    orders: number;
    users: number;
  };
  timeData: {
    dates: string[];
    amounts: number[];
    orders: number[];
    users: number[];
  };
  packageData: {
    names: string[];
    values: number[];
  };
  payMethodData: {
    methods: string[];
    values: number[];
  };
}

const OrderStatistics: React.FC = () => {
  const [loading, setLoading] = useState(false);
  const [dateRange, setDateRange] = useState<[dayjs.Dayjs, dayjs.Dayjs]>([
    dayjs().subtract(30, 'day'),
    dayjs()
  ]);
  const [timeUnit, setTimeUnit] = useState<string>('day');
  const [chartType, setChartType] = useState<string>('amount');
  const [statisticsData, setStatisticsData] = useState<StatisticsData | null>(null);

  // 获取统计数据
  useEffect(() => {
    const fetchStatisticsData = async () => {
      setLoading(true);
      try {
        const params = {
          startDate: dateRange[0].format('YYYY-MM-DD'),
          endDate: dateRange[1].format('YYYY-MM-DD'),
          timeUnit
        };

        const response = await axios.get('/api/orders/statistics', { params });

        if (response.data.code === 200) {
          setStatisticsData(response.data.data);
        }
      } catch (error) {
        console.error('获取统计数据失败:', error);
      } finally {
        setLoading(false);
      }
    };

    fetchStatisticsData();
  }, [dateRange, timeUnit]);

  // 处理日期范围变化
  const handleDateRangeChange = (dates: any) => {
    if (dates && dates.length === 2) {
      setDateRange(dates);
    }
  };

  // 处理时间单位变化
  const handleTimeUnitChange = (value: string) => {
    setTimeUnit(value);
  };

  // 处理图表类型变化
  const handleChartTypeChange = (e: any) => {
    setChartType(e.target.value);
  };

  // 获取趋势图表配置
  const getTrendChartOption = () => {
    if (!statisticsData) return {};

    const { timeData } = statisticsData;
    let seriesData: any[] = [];

    if (chartType === 'amount') {
      seriesData = [
        {
          name: '订单金额',
          type: 'line',
          data: timeData.amounts,
          smooth: true,
          areaStyle: {
            opacity: 0.2
          },
          lineStyle: {
            width: 3
          },
          itemStyle: {
            color: '#1890ff'
          }
        }
      ];
    } else if (chartType === 'orders') {
      seriesData = [
        {
          name: '订单数量',
          type: 'line',
          data: timeData.orders,
          smooth: true,
          areaStyle: {
            opacity: 0.2
          },
          lineStyle: {
            width: 3
          },
          itemStyle: {
            color: '#52c41a'
          }
        }
      ];
    } else if (chartType === 'users') {
      seriesData = [
        {
          name: '用户数量',
          type: 'line',
          data: timeData.users,
          smooth: true,
          areaStyle: {
            opacity: 0.2
          },
          lineStyle: {
            width: 3
          },
          itemStyle: {
            color: '#fa8c16'
          }
        }
      ];
    } else if (chartType === 'all') {
      seriesData = [
        {
          name: '订单金额',
          type: 'line',
          data: timeData.amounts,
          smooth: true,
          lineStyle: {
            width: 2
          },
          itemStyle: {
            color: '#1890ff'
          }
        },
        {
          name: '订单数量',
          type: 'line',
          data: timeData.orders,
          smooth: true,
          lineStyle: {
            width: 2
          },
          itemStyle: {
            color: '#52c41a'
          }
        },
        {
          name: '用户数量',
          type: 'line',
          data: timeData.users,
          smooth: true,
          lineStyle: {
            width: 2
          },
          itemStyle: {
            color: '#fa8c16'
          }
        }
      ];
    }

    return {
      tooltip: {
        trigger: 'axis',
        axisPointer: {
          type: 'cross',
          label: {
            backgroundColor: '#6a7985'
          }
        }
      },
      legend: {
        data: seriesData.map(item => item.name)
      },
      grid: {
        left: '3%',
        right: '4%',
        bottom: '3%',
        containLabel: true
      },
      xAxis: {
        type: 'category',
        boundaryGap: false,
        data: timeData.dates
      },
      yAxis: {
        type: 'value'
      },
      series: seriesData
    };
  };

  // 获取套餐分布图表配置
  const getPackageChartOption = () => {
    if (!statisticsData) return {};

    const { packageData } = statisticsData;

    return {
      tooltip: {
        trigger: 'item',
        formatter: '{a} <br/>{b}: {c} ({d}%)'
      },
      legend: {
        orient: 'vertical',
        left: 10,
        data: packageData.names
      },
      series: [
        {
          name: '套餐销售',
          type: 'pie',
          radius: ['50%', '70%'],
          avoidLabelOverlap: false,
          itemStyle: {
            borderRadius: 10,
            borderColor: '#fff',
            borderWidth: 2
          },
          label: {
            show: false,
            position: 'center'
          },
          emphasis: {
            label: {
              show: true,
              fontSize: '18',
              fontWeight: 'bold'
            }
          },
          labelLine: {
            show: false
          },
          data: packageData.names.map((name, index) => ({
            value: packageData.values[index],
            name
          }))
        }
      ]
    };
  };

  // 获取支付方式图表配置
  const getPayMethodChartOption = () => {
    if (!statisticsData) return {};

    const { payMethodData } = statisticsData;

    return {
      tooltip: {
        trigger: 'axis',
        axisPointer: {
          type: 'shadow'
        }
      },
      grid: {
        left: '3%',
        right: '4%',
        bottom: '3%',
        containLabel: true
      },
      xAxis: {
        type: 'value'
      },
      yAxis: {
        type: 'category',
        data: payMethodData.methods
      },
      series: [
        {
          name: '订单数量',
          type: 'bar',
          data: payMethodData.values,
          itemStyle: {
            color: function(params: any) {
              const colorList = ['#1890ff', '#52c41a', '#fa8c16', '#f5222d', '#722ed1'];
              return colorList[params.dataIndex % colorList.length];
            }
          }
        }
      ]
    };
  };

  return (
    <div>
      <Card>
        <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between' }}>
          <div>
            <RangePicker
              value={dateRange}
              onChange={handleDateRangeChange}
              style={{ marginRight: 16 }}
            />
            <Select
              value={timeUnit}
              onChange={handleTimeUnitChange}
              style={{ width: 120 }}
            >
              <Option value="day">按天</Option>
              <Option value="week">按周</Option>
              <Option value="month">按月</Option>
            </Select>
          </div>
          <Radio.Group value={chartType} onChange={handleChartTypeChange}>
            <Radio.Button value="amount">金额</Radio.Button>
            <Radio.Button value="orders">订单数</Radio.Button>
            <Radio.Button value="users">用户数</Radio.Button>
            <Radio.Button value="all">全部</Radio.Button>
          </Radio.Group>
        </div>

        <Spin spinning={loading}>
          {statisticsData && (
            <>
              <Row gutter={16}>
                <Col span={6}>
                  <Card>
                    <Statistic
                      title="总订单金额"
                      value={statisticsData.totalAmount}
                      precision={2}
                      valueStyle={{ color: '#3f8600' }}
                      prefix="¥"
                      suffix={
                        statisticsData.comparedToLastPeriod.amount > 0 ? (
                          <ArrowUpOutlined style={{ color: '#3f8600' }} />
                        ) : (
                          <ArrowDownOutlined style={{ color: '#cf1322' }} />
                        )
                      }
                    />
                    <div style={{ fontSize: 12, color: '#999', marginTop: 8 }}>
                      较上期
                      <span style={{
                        color: statisticsData.comparedToLastPeriod.amount > 0 ? '#3f8600' : '#cf1322',
                        marginLeft: 4
                      }}>
                        {statisticsData.comparedToLastPeriod.amount > 0 ? '+' : ''}
                        {statisticsData.comparedToLastPeriod.amount}%
                      </span>
                    </div>
                  </Card>
                </Col>
                <Col span={6}>
                  <Card>
                    <Statistic
                      title="总订单数"
                      value={statisticsData.totalOrders}
                      valueStyle={{ color: '#1890ff' }}
                      prefix={<ShoppingCartOutlined />}
                      suffix={
                        statisticsData.comparedToLastPeriod.orders > 0 ? (
                          <ArrowUpOutlined style={{ color: '#3f8600' }} />
                        ) : (
                          <ArrowDownOutlined style={{ color: '#cf1322' }} />
                        )
                      }
                    />
                    <div style={{ fontSize: 12, color: '#999', marginTop: 8 }}>
                      较上期
                      <span style={{
                        color: statisticsData.comparedToLastPeriod.orders > 0 ? '#3f8600' : '#cf1322',
                        marginLeft: 4
                      }}>
                        {statisticsData.comparedToLastPeriod.orders > 0 ? '+' : ''}
                        {statisticsData.comparedToLastPeriod.orders}%
                      </span>
                    </div>
                  </Card>
                </Col>
                <Col span={6}>
                  <Card>
                    <Statistic
                      title="下单用户数"
                      value={statisticsData.totalUsers}
                      valueStyle={{ color: '#fa8c16' }}
                      prefix={<UserOutlined />}
                      suffix={
                        statisticsData.comparedToLastPeriod.users > 0 ? (
                          <ArrowUpOutlined style={{ color: '#3f8600' }} />
                        ) : (
                          <ArrowDownOutlined style={{ color: '#cf1322' }} />
                        )
                      }
                    />
                    <div style={{ fontSize: 12, color: '#999', marginTop: 8 }}>
                      较上期
                      <span style={{
                        color: statisticsData.comparedToLastPeriod.users > 0 ? '#3f8600' : '#cf1322',
                        marginLeft: 4
                      }}>
                        {statisticsData.comparedToLastPeriod.users > 0 ? '+' : ''}
                        {statisticsData.comparedToLastPeriod.users}%
                      </span>
                    </div>
                  </Card>
                </Col>
                <Col span={6}>
                  <Card>
                    <Statistic
                      title="平均订单金额"
                      value={statisticsData.averageAmount}
                      precision={2}
                      valueStyle={{ color: '#722ed1' }}
                      prefix="¥"
                      suffix={<LineChartOutlined />}
                    />
                  </Card>
                </Col>
              </Row>

              <Divider />

              <Card title="订单趋势">
                <ReactECharts
                  option={getTrendChartOption()}
                  style={{ height: 400 }}
                />
              </Card>

              <Divider />

              <Row gutter={16}>
                <Col span={12}>
                  <Card title="套餐销售分布">
                    <ReactECharts
                      option={getPackageChartOption()}
                      style={{ height: 400 }}
                    />
                  </Card>
                </Col>
                <Col span={12}>
                  <Card title="支付方式分布">
                    <ReactECharts
                      option={getPayMethodChartOption()}
                      style={{ height: 400 }}
                    />
                  </Card>
                </Col>
              </Row>
            </>
          )}
        </Spin>
      </Card>
    </div>
  );
};

export default OrderStatistics;
